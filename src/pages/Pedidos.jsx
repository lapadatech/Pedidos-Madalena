...
function Pedidos() {
  ...
  const [filtroPagamento, setFiltroPagamento] = useState(dashboardFilters?.filtroPagamento || 'todos');
  ...
  const handleExportExcel = async () => {
    ...
    try {
        ...
        const dataToExport = allPedidos.map(p => ({
          ...
          'Data': p.data_entrega ? new Date(p.data_entrega + 'T00:00:00').toLocaleDateString('pt-BR') : '-',
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {podeCriar && (
                <Button className="bg-orange-500 hover:bg-orange-600" onClick={() => setWizardAberto(true)}>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <div className="flex justify-between items-center border-b">
                <TabsList className="bg-transparent border-none p-0">
                    <TabsTrigger value="kanban" className="data-[state=active]:shadow-none data-[state=active]:border-b-2 border-orange-500 rounded-none">Kanban</TabsTrigger>
                    <TabsTrigger value="tabela" className="data-[state=active]:shadow-none data-[state=active]:border-b-2 border-orange-500 rounded-none">Tabela</TabsTrigger>
                </TabsList>
            </div>
            ...
            {activeTab === 'tabela' && (
             <div className="p-4 bg-white rounded-b-lg shadow flex flex-col sm:flex-row flex-wrap gap-4">
                ...
                <div className="w-full sm:w-auto sm:min-w-[200px]">
                  <TagSelector
                    selectedTagIds={filtroTagIds}
                    onChange={setFiltroTagIds}
                  />
                </div>
            </div>
            )}
            ...
            <TabsContent value="kanban" className={activeTab === 'kanban' ? 'mt-4' : 'mt-0'}>
                <PedidosKanban pedidos={pedidos} onAbrirDetalhe={handleAbrirDetalhe} loading={loading} />
            </TabsContent>
            <TabsContent value="tabela" className="mt-0">
                <PedidosTabela pedidos={pedidos} onAbrirDetalhe={handleAbrirDetalhe} loading={loading} />
        ...
        <Dialog open={wizardAberto} onOpenChange={open => { if (!open) resetWizard(); }}>
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>{editMode ? 'Editar Pedido' : 'Novo Pedido'} - Etapa {etapaAtual} de 3</DialogTitle>
                </DialogHeader>
                {etapaAtual === 1 && <CriarPedidoStep1 onProximo={handleProximaEtapa} dadosIniciais={dadosPedido} />}
                {etapaAtual === 2 && <CriarPedidoStep2 onProximo={handleProximaEtapa} onVoltar={handleVoltarEtapa} dadosIniciais={dadosPedido} />}
                {etapaAtual === 3 && <CriarPedidoStep3 onVoltar={handleVoltarEtapa} onFinalizar={handleFinalizarPedido} dadosIniciais={dadosPedido} editMode={editMode} />}
            </DialogContent>
        </Dialog>

        {detalheDialogAberto && <PedidoDetalheDialog pedidoId={pedidoSelecionadoId} open={detalheDialogAberto} onOpenChange={handleFecharDetalhe} onIniciarEdicao={handleIniciarEdicao} onPedidoDeletado={handlePedidoDeletado} />}

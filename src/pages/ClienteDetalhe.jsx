      ...
      <div className="space-y-6">
        <div>
          <Button asChild variant="ghost" className="mb-4">
            <Link to="/clientes"><ArrowLeft className="mr-2 h-4 w-4" /> Voltar para Clientes</Link>
          </Button>
        ...
        <div className="bg-white p-6 rounded-lg shadow space-y-4">
          <h2 className="text-xl font-semibold flex items-center gap-2"><Home className="text-orange-500"/> Endereços</h2>
          {enderecos.length > 0 ? (
            <ul className="space-y-4">
              {enderecos.map((end) => (
                <li key={end.id} className="p-4 border rounded-md">
                  {end.principal && <span className="text-xs font-bold text-orange-500 uppercase">Principal</span>}
                  <p className="font-medium">{end.rua}, {end.numero} {end.complemento && `- ${end.complemento}`}</p>
        ...
        <div className="bg-white p-6 rounded-lg shadow space-y-4">
            <h2 className="text-xl font-semibold flex items-center gap-2"><Package className="text-orange-500"/> Histórico de Pedidos</h2>
            {pedidos.length > 0 ? (
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nº do Pedido</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {pedidos.map((pedido) => (
                                <tr key={pedido.id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(pedido.data_entrega).toLocaleDateString('pt-BR')}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">R$ {pedido.total.toFixed(2).replace('.', ',')}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>

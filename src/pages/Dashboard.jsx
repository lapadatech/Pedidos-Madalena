...
import { Users, Package, DollarSign, AlertCircle, Truck, AlertTriangle, PackageCheck, ListOrdered, Coins as HandCoins, CalendarDays, Calendar as CalendarIcon } from 'lucide-react';
...
function Dashboard() {
  ...
  const [stats, setStats] = useState([
    { id: 'totalPedidos', title: 'Total de\nPedidos', value: '0', subtitle: 'R$ 0,00 total', icon: ListOrdered, color: 'bg-green-500', loading: true, bgColor: 'bg-green-50' },
    { id: 'naoPagos', title: 'Pedidos a\nReceber', value: '0', subtitle: 'R$ 0,00 pendentes', icon: DollarSign, color: 'bg-red-500', loading: true, bgColor: 'bg-red-50' },
    { id: 'naoEntregues', title: 'Pedidos Não\nEntregues', value: '0', subtitle: 'R$ 0,00 pendentes', icon: Truck, color: 'bg-yellow-500', loading: true, bgColor: 'bg-yellow-50' },
    { id: 'entreguesNaoPagos', title: 'Pedidos Entregues\ne Não Pagos', value: '0', subtitle: 'R$ 0,00 pendentes', icon: HandCoins, color: 'bg-purple-500', loading: true, bgColor: 'bg-purple-50' },
    { id: 'pagosNaoEntregues', title: 'Pedidos Pagos e\nNão Entregues', value: '0', subtitle: 'R$ 0,00 pendentes', icon: PackageCheck, color: 'bg-blue-600', loading: true, bgColor: 'bg-blue-50' },
    { id: 'atrasados', title: 'Pedidos\nAtrasados', value: '0', subtitle: 'R$ 0,00 atrasados', icon: AlertTriangle, color: 'bg-orange-600', loading: true, bgColor: 'bg-orange-50' },
    { id: 'clientes', title: 'Clientes\nCadastrados', value: '0', icon: Users, color: 'bg-blue-500', loading: true, bgColor: 'bg-white' },
    { id: 'produtos', title: 'Produtos\nAtivos', value: '0', icon: Package, color: 'bg-green-500', loading: true, bgColor: 'bg-white' },
  ]);
  ...
  const carregarDados = useCallback(async () => {
    try {
      ...
      const [totalPedidosGeral, pedidosAbertosResult, totalClientes, totalProdutosAtivos, pedidosGeraisResult] = await Promise.all([
      ...
      const pedidosNaoPagos = todosPedidosNoPeriodo.filter(p => p.status_pagamento === 'Não Pago');
      const totalNaoPago = pedidosNaoPagos.reduce((acc, p) => acc + (p.total || 0), 0);

      // 3. Pedidos Não Entregues (Status Entrega != Entregue)
      const pedidosNaoEntregues = todosPedidosNoPeriodo.filter(p => p.status_entrega === 'Não Entregue');
      const totalNaoEntregue = pedidosNaoEntregues.reduce((acc, p) => acc + (p.total || 0), 0);

      // 4. Pedidos Pagos E Não Entregues
      const pedidosPagosNaoEntregues = todosPedidosNoPeriodo.filter(p => p.status_pagamento === 'Pago' && p.status_entrega !== 'Entregue');
      const totalPagosNaoEntregues = pedidosPagosNaoEntregues.reduce((acc, p) => acc + (p.total || 0), 0);

      // 5. Pedidos Entregues E Não Pagos
      const pedidosEntreguesNaoPagos = todosPedidosNoPeriodo.filter(p => p.status_pagamento !== 'Pago' && p.status_entrega === 'Entregue');
      const totalEntreguesNaoPagos = pedidosEntreguesNaoPagos.reduce((acc, p) => acc + (p.total || 0), 0);
      ...
      const clientesPendentesList = Object.values(clientesMap).sort((a, b) => b.valorTotal - a.valorTotal);
      setClientesPendentes(clientesPendentesList);
      setLoadingClientes(false);
      
      setStats([
        { id: 'totalPedidos', title: 'Total de\nPedidos', value: totalPedidosCount, subtitle: `${formatCurrency(totalPedidosValue)} total`, icon: ListOrdered, color: 'bg-green-500', loading: false, bgColor: 'bg-green-50' },
        { id: 'naoPagos', title: 'Pedidos a\nReceber', value: pedidosNaoPagos.length, subtitle: `${formatCurrency(totalNaoPago)} pendentes`, icon: DollarSign, color: 'bg-red-500', loading: false, bgColor: 'bg-red-50' },
        { id: 'naoEntregues', title: 'Pedidos Não\nEntregues', value: pedidosNaoEntregues.length, subtitle: `${formatCurrency(totalNaoEntregue)} pendentes`, icon: Truck, color: 'bg-yellow-500', loading: false, bgColor: 'bg-yellow-50' },
        { id: 'entreguesNaoPagos', title: 'Pedidos Entregues\ne Não Pagos', value: pedidosEntreguesNaoPagos.length, subtitle: `${formatCurrency(totalEntreguesNaoPagos)} pendentes`, icon: HandCoins, color: 'bg-purple-500', loading: false, bgColor: 'bg-purple-50' },
        { id: 'pagosNaoEntregues', title: 'Pedidos Pagos e\nNão Entregues', value: pedidosPagosNaoEntregues.length, subtitle: `${formatCurrency(totalPagosNaoEntregues)} pendentes`, icon: PackageCheck, color: 'bg-blue-600', loading: false, bgColor: 'bg-blue-50' },
        { id: 'atrasados', title: 'Pedidos\nAtrasados', value: pedidosAtrasados.length, subtitle: `${formatCurrency(totalAtrasado)} atrasados`, icon: AlertTriangle, color: 'bg-orange-600', loading: false, bgColor: 'bg-orange-50' },
        { id: 'clientes', title: 'Clientes\nCadastrados', value: totalClientes, icon: Users, color: 'bg-blue-500', loading: false, bgColor: 'bg-white' }, // Total geral
        { id: 'produtos', title: 'Produtos\nAtivos', value: totalProdutosAtivos, icon: Package, color: 'bg-green-500', loading: false, bgColor: 'bg-white' }, // Total geral
      ]);
      ...
      const errorState = { value: <AlertCircle className="h-6 w-6 text-red-500"/>, isError: true, loading: false };
  ...
  const handleCardClick = (cardId) => {
    ...
    if (cardId === 'naoPagos') {
      ...
      navigate('/pedidos', { state: { filtroPagamento: 'Pago', filtroEntrega: 'Não Entregue', filtroStatus: 'abertos' } });
    } else if (cardId === 'entreguesNaoPagos') {
      navigate('/pedidos', { state: { filtroPagamento: 'Não Pago', filtroEntrega: 'Entregue', filtroStatus: 'abertos' } });
  ...
  return (
    <>
      ...
      <div className="space-y-8">
                    ...
                    <PopoverContent className="w-80 p-4" align="end">
                        <div className="grid gap-4">
                            ...
                            <div className="grid gap-2">
                                ...
                                <Button className="w-full mt-2 bg-primary hover:bg-primary/90" onClick={handleApplyCustomDate}>
                                    Aplicar Filtro
                                </Button>
                            </div>
                        </div>
          ...
          {stats.map((stat, index) => (
            ...
            >
              <div className="flex items-start justify-between">
                <p className="text-sm text-gray-700 font-semibold whitespace-pre-line leading-tight" title={stat.title.replace('\n', ' ')}>
                  {stat.title}
                </p>
              ...
              <div className="mt-3">
                <div className="text-3xl font-bold text-gray-900">
                  {stat.loading ? <div className="h-8 w-24 bg-gray-200 rounded animate-pulse"></div> : (stat.isError ? stat.value : <p>{stat.value}</p>)}
                </div>
                {stat.subtitle && !stat.loading && <p className="text-xs text-gray-600 mt-1">{stat.subtitle}</p>}
                 {stat.loading && stat.subtitle && <div className="h-4 w-32 mt-1 bg-gray-200 rounded animate-pulse"></div>}
              </div>
            </motion.div>
          ))}
        </div>
        ...
        >
          <div className="p-6">
            <h3 className="text-lg font-semibold text-gray-900">Clientes com Pagamentos Pendentes</h3>
              ...
              <TableBody>
                {loadingClientes ? (
                  Array.from({ length: 5 }).map((_, i) => (
                    <TableRow key={i}>
                      <TableCell className="px-6"><div className="h-5 w-32 bg-gray-200 rounded animate-pulse"></div></TableCell>
                      <TableCell><div className="h-5 w-10 mx-auto bg-gray-200 rounded animate-pulse"></div></TableCell>
                      <TableCell><div className="h-5 w-20 ml-auto bg-gray-200 rounded animate-pulse"></div></TableCell>
                      <TableCell className="px-6"><div className="h-5 w-20 ml-auto bg-gray-200 rounded animate-pulse"></div></TableCell>
                    </TableRow>
                  ))
                  ...
                  clientesPendentes.slice(0, 5).map((cliente) => (
                    <TableRow key={cliente.id}>
                      ...
                      <TableCell className="text-right px-6">{formatDate(cliente.ultimoPedido)}</TableCell>
                    </TableRow>
                  ))
                ) : (
                  <TableRow>
          ...
          {clientesPendentes.length > 5 && (
              <div className="text-right p-4 border-t">
                  <Button variant="link" onClick={handleVerTodosPendentes}>Ver todos ({clientesPendentes.length})</Button>
              </div>
          )}
        </motion.div>
      </div>
